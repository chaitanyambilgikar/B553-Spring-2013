...
SDoublePlane Phi_calculator(vector <vector<Coordinate> > &coor_map, Coordinate mean, Coordinate var, SDoublePlane my_potentials,int rows, int cols) //returns an array of coordinates, which is the best possible assignments for two parts i and j
{
  ...
  for(int ix = 0; ix<rows; ix+=3)
  {
    for(int iy = 0; iy<cols; iy+=3) //outer two loops iterate all possible coordinates for the parent
    {
      double max_phi = -1e100;
      Coordinate max_coord;
      max_coord.row = 0;
      max_coord.col = 0;
      for (int jx = 0; jx<rows; jx+=3)      //inner two loops iterate through all possible coordinates of child, for each of parents value
      {
        for (int jy = 0; jy<cols; jy+=3)
        {
...
//takes all training data loaded by load_ground_truth and means and vars meade by learning(). Also takes mypotentials which are already ready in main from where this method will be called.
  ...
  answer.coords[1].row = coor_map_le_re[answer.coords[0].row][answer.coords[0].col].row;
  answer.coords[1].col = coor_map_le_re[answer.coords[0].row][answer.coords[0].col].col;
 
  answer.coords[2].row = coor_map_le_n[answer.coords[0].row][answer.coords[0].col].row;
  answer.coords[2].col = coor_map_le_n[answer.coords[0].row][answer.coords[0].col].col;

  answer.coords[3].row = coor_map_le_lm[answer.coords[0].row][answer.coords[0].col].row;
  answer.coords[3].col = coor_map_le_lm[answer.coords[0].row][answer.coords[0].col].col;

  answer.coords[4].row = coor_map_le_rm[answer.coords[0].row][answer.coords[0].col].row;
  answer.coords[4].col = coor_map_le_rm[answer.coords[0].row][answer.coords[0].col].col;

  answer.coords[5].row = coor_map_le_c[answer.coords[0].row][answer.coords[0].col].row;
  answer.coords[5].col = coor_map_le_c[answer.coords[0].row][answer.coords[0].col].col;
...
SDoublePlane Phi_calculator_loopy(Coordinate mean, Coordinate var,SDoublePlane my_potential,int rows, int cols) //returns an array of coordinates, which is the best possible assignments for two parts i and j
{
  ...
  for(int ix = 0; ix<rows; ix+=3)
  {
    for(int iy = 0; iy<cols; iy+=3)
    {
      double max_phi = -1e100;
      for (int jx = 0; jx<rows; jx+=3)
      {
        for (int jy = 0; jy<cols; jy+=3)
        {
          double t1 = pow(ix-jx-mean.row,2)/(2*var.row);
